// student/Profile.jsx - Converted from student/profile.html & student/js/profile.js
...
export default function StudentProfile() {
    ...
    const displayName = userData?.fullName || userData?.firstName || user?.email?.split('@')[0] || 'Student';
        ...
        <div style={{ padding: 30, background: '#f8fafc', minHeight: '100vh' }}>
            <div style={{ maxWidth: 750, margin: '0 auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                    <div>
                        <h2 style={{ margin: 0, color: '#1B2A4A', fontSize: 22 }}>Student Profile</h2>
                        <span style={{ fontSize: 13, color: '#64748b' }}>Manage your account and view personal info</span>
                    </div>
                ...
                <div style={{ background: '#1B2A4A', color: '#fff', borderRadius: 16, padding: 24, marginBottom: 24, border: '2px solid #FFD700' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                        ...
                        <div>
                            ...
                            <p style={{ margin: 0, color: '#cbd5e1', fontSize: 13 }}>LRN: {userData?.lrn || '123456789012'} • Grade 7 - Diamond</p>
                ...
                <div style={{ background: '#fff', borderRadius: 12, padding: 24, border: '1px solid #e2e8f0', marginBottom: 24 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: '#1B2A4A', marginBottom: 16 }}>Personal Details</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, fontSize: 13 }}>
                        ...
                        <div>
                            ...
                            <strong>{userData?.birthdate || 'August 15, 2009'}</strong>
                        </div>
                        <div>
                            <span style={{ color: '#64748b', display: 'block', fontSize: 11 }}>Gender</span>
                            <strong>{userData?.gender || 'Male'}</strong>
                        </div>
                        <div>
                            <span style={{ color: '#64748b', display: 'block', fontSize: 11 }}>Account Role</span>
                            <strong>{userData?.role || 'Student'}</strong>
                        </div>
                    </div>
                </div>
